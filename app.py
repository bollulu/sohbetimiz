...
def block_user(data):
    ...
    current_blocks = json.loads(current_user.blocked_list)
    if target not in current_blocks:
        current_blocks.append(target)
        current_user.blocked_list = json.dumps(current_blocks)
        db.session.commit()
