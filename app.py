...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'wp_ultra_mobile_2026'
...
def handle_offer(data): emit('video-offer', data, broadcast=True, include_self=False)

@socketio.on('video-answer')
def handle_answer(data): emit('video-answer', data, broadcast=True, include_self=False)

@socketio.on('new-ice-candidate')
def handle_candidate(data): emit('new-ice-candidate', data, broadcast=True, include_self=False)
