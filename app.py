...
class Story(db.Model):
    ...
    username = db.Column(db.String(50))
    user_avatar = db.Column(db.Text)
    content = db.Column(db.Text)       # GÃ¶rsel veya Video verisi
    audio_data = db.Column(db.Text)    # Arka plan mÃ¼ziÄŸi (Varsa)
...
def add_story(data):
    # Veri paketinden gÃ¶rseli ve opsiyonel mÃ¼ziÄŸi alÄ±yoruz
    new_story = Story(
        username=current_user.username, 
        user_avatar=current_user.avatar, 
        content=data['content'],
        audio_data=data.get('music'), # MÃ¼zik varsa kaydet, yoksa None
...
def send_stories():
    ...
    for s in stories:
        ...
        grouped[s.username]['items'].append({
            'id': s.id,
            'content': s.content,
            'music': s.audio_data, # MÃ¼ziÄŸi de gÃ¶nderiyoruz
            'type': s.media_type,
